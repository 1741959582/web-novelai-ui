...
// weight syntax `{}` / `[]`, numeric `2::tag::`, and grouping `()` are part of
// NovelAI's prompt language and are preserved. Optional "wildcard" groups (anything containing a
// `|`, plus `${...}` / `<...>`) are shielded from every transform.

import { splitPromptTags } from "@/utils/promptWeight";
...
export function normalizePrompt(input: string, opts: NormalizeOptions): string {
  ...
  // 3. Per-tag cleanup. splitPromptTags keeps `2::a, b::` and `{a, b}` intact.
  let tags = splitPromptTags(text);
